...
int type;
int punkty;
int mode = 1; // 0 - start, 1 - play, 2 - end
blockColor colors[4];
...
int main()
{
	...
	//button buttons[4]; //0 - start game, 1 - help, 2 - exit, 3 - play again
	...
	while (sfRenderWindow_isOpen(window))
	{
		...
		else if(mode == 2)
		{
			...
			{
				deleteAll(window, background, shapes, text_result, font, new_game, resultsBackground);
				exit(0);
			} // Q - quit
		}
